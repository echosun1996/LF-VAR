...
class VAR(nn.Module):
    def __init__(
        ...
        self.radiomics_encoder = nn.Sequential(
            nn.Linear(radiomics_feature_dim, self.C),
            nn.LayerNorm(self.C),
            nn.SiLU()
        )
    ...
    def autoregressive_infer_cfg(
            ...
            save_path:str = None,img_path:str = None, radiomics_features:torch.Tensor = None
    ) -> torch.Tensor:
        if g_seed is None:
            rng = None
        else:
        ...
        radiomics_features = radiomics_features.to(device=label_B.device)
        radiomics_embedding = self.radiomics_encoder(radiomics_features)
        
        cond_class = self.class_emb(label_B)
        uncond_class = self.class_emb(torch.full_like(label_B, fill_value=self.num_classes))
        
        combined_embedding = torch.cat((
            cond_class + radiomics_embedding,
            uncond_class + radiomics_embedding
        ), dim=0)
        
        sos = cond_BD = combined_embedding
        ...
        for si, pn in enumerate(self.patch_nums):
            ...
            def replace_embedding(edit_mask: torch.Tensor, h_BChw: torch.Tensor, gt_BChw: torch.Tensor, ph: int,
                                  pw: int) -> torch.Tensor:
                B = h_BChw.shape[0]
                h, w = edit_mask.shape[-2:]
                if edit_mask.ndim == 2:
                    edit_mask = edit_mask.unsqueeze(0).expand(B, h, w)
                assert ph > 0 and pw > 0, "ph and pw must be positive"
                force_gt_B1hw = F.interpolate(
                    edit_mask[0].to(dtype=torch.float, device=gt_BChw.device).unsqueeze(1),
                    size=(ph, pw),
                    mode='bilinear',
                    align_corners=False
                ).gt(0.5).int()
            ...
            if edit_mask is not None:
                gt_BChw = self.vae_quant_proxy[0].embedding(input_img_tokens[si]).transpose_(1, 2).reshape(B, self.Cvae,
                                                                                                           pn, pn)
    ...
    def forward(self, label_B: torch.LongTensor, x_BLCv_wo_first_l: torch.Tensor, radiomics_features: torch.Tensor) -> torch.Tensor:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            class_embedding = self.class_emb(label_B)
            combined_embedding = class_embedding + radiomics_embedding
