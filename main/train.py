...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        if L_tail != -1: best_L_tail, best_acc_tail = min(best_L_mean, L_tail), max(best_acc_tail, acc_tail)
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    for it, (inp, mask, label, path, radiomics) in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 8000 else 5, header):
        ...
        grad_norm, scale_log2 = trainer.train_step(
            ...
            inp_B3HW=inp, label_B=label, prog_si=prog_si, prog_wp_it=args.pgwp * iters_train, radiomics_features=radiomics
        )
