...
def main_training():
    ...
    L_mean, L_tail = -1, -1
    for ep in range(start_ep, args.ep+1):
        ...
        args.remain_time, args.finish_time = 0, 0

        is_val_and_also_saving = True
        if is_val_and_also_saving:
            val_loss_mean, val_loss_tail, val_acc_mean, val_acc_tail, tot, cost = trainer.infer(ld_val,args)
...
def main_inference():
    ...
    trainer.var_wo_ddp.eval()
    with torch.no_grad():
        for it, (inp_B3HW, label_B) in enumerate(ld_val):

            inp_B3HW = inp_B3HW.to(args.device, non_blocking=True)
            label_B = label_B.to(args.device, non_blocking=True)


            B, V = label_B.shape[0], trainer.vae_local.vocab_size
            inp_B3HW = inp_B3HW.to(dist.get_device(), non_blocking=True)
            label_B = label_B.to(dist.get_device(), non_blocking=True)

            gt_idx_Bl: List[ITen] = trainer.vae_local.img_to_idxBl(inp_B3HW)
            gt_BL = torch.cat(gt_idx_Bl, dim=1)
            x_BLCv_wo_first_l: Ten = trainer.quantize_local.idxBl_to_var_input(gt_idx_Bl)

            trainer.var_wo_ddp.forward
            logits_BLV = trainer.var_wo_ddp(label_B, x_BLCv_wo_first_l).data
            print(len(logits_BLV))
            print("############")

            for idx in range(logits_BLV.size(0)):
                image_tensor = logits_BLV[idx].cpu()

                transform_to_image = transforms.ToPILImage()
                image_tensor_clamped = torch.clamp(image_tensor, 0, 1)
                image = transform_to_image(image_tensor_clamped)

                image = image.resize((512, 512), Resampling.LANCZOS)

                lbl = label_B[idx].item()
                folder_path = os.path.join(args.local_out_dir_path, str(lbl))
                os.makedirs(folder_path, exist_ok=True)
                file_path = os.path.join(folder_path, f"{it}_{idx}.png")
                image.save(file_path)



            if it % 10 == 0:
                print(f'[Inference] Processed batch {it + 1}/{len(ld_val)}')
